...
{
	m_vecWorldBordersFrom.X = -16000.0f;
	m_vecWorldBordersFrom.Y = -16000.0f;
	m_vecWorldBordersFrom.Z = -5000.0f;
...
{
	/*if(pChatWindow) {
		pChatWindow->AddDebugMessage("CNetGame::ProcessPools");
	}*/

	if(m_pPlayerPool) 
	{
		/*if(pChatWindow) {
			pChatWindow->AddDebugMessage("CPlayerPool::Process");
		}*/
	...
	{
		/*if(pChatWindow) {
			pChatWindow->AddDebugMessage("CVehiclePool::Process");
		}*/
	...
	{
		/*if(pChatWindow) {
			pChatWindow->AddDebugMessage("CPickupPool::Process");
		}*/
...
void CNetGame::UpdateNetwork()
{
	...
	while(pkt = m_pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			Packet_ConnectionAttemptFailed(pkt);
			break;

			case ID_NO_FREE_INCOMING_CONNECTIONS:
			Packet_NoFreeIncommingConnections(pkt);
			break;
...
void CNetGame::Packet_ConnectionAttemptFailed(Packet* pkt)
{
	if(pChatWindow) {
...
void CNetGame::Packet_NoFreeIncommingConnections(Packet* pkt)
{
	if(pChatWindow) {
