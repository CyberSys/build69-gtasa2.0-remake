...
void ServerJoin(RPCParameters *rpcParams)
{
	...
	FLog("New player: %s[%i] - NPC: %d", szPlayerName, playerId, bIsNPC);
}
...
void ServerQuit(RPCParameters *rpcParams)
{
	...
	FLog("Delete player: %i. Reason: %d", playerId, byteReason);
}

void ClientMessage(RPCParameters *rpcParams)
{
	//FLog("RPC: ClientMessage");
	unsigned char* Data = reinterpret_cast<unsigned char *>(rpcParams->input);
	int iBitLength = rpcParams->numberOfBitsOfData;
	...
	//Log(szMsg);
	if(pChatWindow) pChatWindow->AddClientMessage(dwColor, szMsg);
...
void Chat(RPCParameters *rpcParams)
{
	...
	if (playerId == pPlayerPool->GetLocalPlayerID())
	{
		...
		if (pLocalPlayer) 
		{
			if(pChatWindow) pChatWindow->AddChatMessage(pPlayerPool->GetLocalPlayerName(),
...
{
	FLog("RPC: DisableRaceCheckpoint");
