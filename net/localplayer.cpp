...
bool CLocalPlayer::HandlePassengerEntry()
{
    ...
    // int isHoldDown = (( int (*)(int, int, int))(g_GTASAAdr+0x270818+1))(0, 1, 1);
...
{
	FLog("CLocalPlayer::UpdateRemoteInterior %d", byteInterior);
...
bool CLocalPlayer::Spawn()
{
	...
	// CCamera::Fade
	ARMHook::writeMemory(g_GTASAAdr+0x3DB5E0, (uintptr_t)"\x70\x47", 2); // bx lr
...
{
	m_byteSpecialAction = byteSpecialAction;
}
